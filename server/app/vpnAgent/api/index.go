...
func (c vpnAgentApi) InstallAgentSh(r *ghttp.Request) {
	...
	sh = strings.Replace(sh, "{{settings.serverApiUrl}}", library.GetAgentAPI(library.IsIPv6(req.ServerIp)), 1)
	sh = strings.Replace(sh, "{{frpCommon.startFrps}}", "true", 1)
	sh = strings.Replace(sh, "{{xrayCommon.commonUuid}}", strings.TrimSpace(library.Settings.Agent.CommonUUID), 1)
	sh = strings.Replace(sh, "{{settings.mqttServer}}", library.GetAgentMqtt(library.IsIPv6(req.ServerIp)), 1)
	...
	sh = strings.ReplaceAll(sh, "{{server.docker}}", fmt.Sprintf("%s/agent/install/docker/main.sh", library.GetAgentAPI(library.IsIPv6(req.ServerIp))))
	sh = strings.ReplaceAll(sh, "{{server.mirrors}}", fmt.Sprintf("%s/agent/install/mirrors/main.sh", library.GetAgentAPI(library.IsIPv6(req.ServerIp))))
	sh = strings.ReplaceAll(sh, "{{agent_api}}", library.GetAgentAPI(library.IsIPv6(req.ServerIp)))
