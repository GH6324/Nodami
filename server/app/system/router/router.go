...
func CacheControlMiddleware(r *ghttp.Request) {
	if strings.HasSuffix(r.Request.URL.Path, ".js") ||
		strings.HasSuffix(r.Request.URL.Path, ".css") ||
		strings.HasSuffix(r.Request.URL.Path, ".jpg") ||
		strings.HasSuffix(r.Request.URL.Path, ".png") {
		r.Response.Header().Set("Cache-Control", "public, max-age="+strconv.Itoa(60*60))
	}
	r.Middleware.Next()
}
func init() {
	s := g.Server()
	//adminPath := "./public/admin"
	//s.Use(CacheControlMiddleware)
	//s.AddStaticPath("/admin", adminPath)
	//s.AddStaticPath("/serverStatic", adminPath+"/serverStatic")

	s.Group("/", func(group *ghttp.RouterGroup) {

		serverRoot := "/" + g.Cfg().GetString("server.ServerRoot")
		group.GET(serverRoot+"*path", func(r *ghttp.Request) {
		...
		group.Group("/system", func(group *ghttp.RouterGroup) {
			...
			group.Group("/config", func(group *ghttp.RouterGroup) {
				...
				group.GET("/configKey/:configKey", api.SysWebSet.ConfigKey)
				//站点设置
			...
			group.Group("/dashboard", func(group *ghttp.RouterGroup) {
				group.GET("info", api.SysDashboard.Info)
				group.GET("chart", api.SysDashboard.Chart)
