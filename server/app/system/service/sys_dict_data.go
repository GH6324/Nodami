...
func (s sysDictData) DictDataAllList(dictType string) (list []*model.SysDictData, err error) {
	d := g.DB().Model(dao.SysDictData.Table)
	d = d.Where(dao.SysDictData.Columns.DictType+" = ?", dictType)
	err = d.Order(dao.SysDictData.Columns.DictSort + " asc," + dao.SysDictData.Columns.DictCode + " asc").Scan(&list)
	if err != nil {
		g.Log().Error(err)
		err = gerror.New("获取数据失败")
		return
