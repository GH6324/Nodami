...
func (s *sysConfig) SelectListByPage(req *model.SysConfigSearchReq) (total, page int, list []*model.SysConfig, err error) {
	...
	err = m.Page(page, req.PageSize).Order("config_id desc").Scan(&list)
	if err != nil {
		g.Log().Error(err)
...
func (s *sysConfig) GetConfigByKey(key string) (config *model.SysConfig, err error) {
	...
	cache := Cache.New()
	cacheKey := global.SysConfigTag + key
	cf := cache.Get(cacheKey)
	if cf != nil {
	...
	if config != nil {
		cache.Set(cacheKey, config, 60*time.Second)
	}
	return
}
...
// GetHostsNot
func (s *sysConfig) GetHostsNot() (hosts map[string]bool) {
	cache := Cache.New()
	hosts = map[string]bool{}

	cf := cache.Get(global.SysConfigTag + "hosts_not")
	if cf != nil {
		gconv.Struct(cf, &hosts)
		return
	}

	var config *model.SysConfig
	config, err := s.GetByKey("hosts_not")
	if err != nil {
		return
	}
	list := strings.Split(config.Remark, "\n")
	for _, v := range list {
		hosts[v] = true
	}

	if config != nil {
		cache.Set(global.SysConfigTag+"hosts_not", hosts, 60*time.Second, global.SysConfigTag)
	}
